...
contextBridge.exposeInMainWorld("weilaijia", {
  ...
  shell: {
    ...
    openTab: (title?: string) => ipcRenderer.invoke("shell:tabs.open", title),
    focusTab: (tabId: string) => ipcRenderer.invoke("shell:tabs.focus", tabId),
    closeTab: (tabId: string) => ipcRenderer.invoke("shell:tabs.close", tabId),
