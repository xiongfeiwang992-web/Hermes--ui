...
async function houseSourceSelectHtml(selected = "", includeEmpty = true) {
  const result = await api("config.houseSources", {});
  const sources = result.ok
    ? (result.data as Array<{ value: string; label: string }>)
    : [
        { value: "门店到访", label: "门店到访" },
...
async function propertyTypeSelectHtml(selected = "residential", includeEmpty = false) {
  const result = await api("config.propertyTypes", {});
  const types = result.ok
    ? (result.data as Array<{ value: string; label: string }>)
    : [
  ...
  const options = types
    ...
    .join("");
  return includeEmpty ? `<option value="">全部物业</option>${options}` : options;
}
...
async function renderHouses(main: HTMLElement) {
  ...
  main.innerHTML = `
    ...
    <div class="filters">
      ...
      <select data-f="property_type"><option value="">全部物业</option></select>
  ...
  const propertyFilter = main.querySelector("[data-f=property_type]") as HTMLSelectElement;
  propertyFilter.innerHTML = await propertyTypeSelectHtml("", true);
  ...
  main.querySelector("[data-new]")!.addEventListener("click", async () => {
    ...
    const sourceOptions = await houseSourceSelectHtml("");
    const propertyOptions = await propertyTypeSelectHtml("residential");
    openDialog(
      "新建房源",
      `
      <label>标题<input name="title" required /></label>
      <label>类型<select name="deal_type"><option value="sale">出售</option><option value="rent">出租</option></select></label>
      <label>物业<select name="property_type">${propertyOptions}</select></label>
  ...
  if (sourceFilter) {
    sourceFilter.innerHTML = `<option value="">全部来源</option>${await houseSourceSelectHtml("", false)}`;
  }
  main.querySelectorAll("[data-f]").forEach((input) =>
...
async function renderCustomers(main: HTMLElement) {
  ...
  main.innerHTML = `
    ...
    <div class="filters">
      ...
      <select data-f="status"><option value="">全部状态</option><option value="new">待联系</option><option value="following">跟进中</option><option value="viewing">带看中</option><option value="suspended">暂缓</option><option value="deal_pending">成交中</option><option value="closed">已成交</option><option value="invalid">无效</option><option value="public_pool">公客池</option></select>
    ...
    container.innerHTML = `
      ...
      ${listBlock(
        ...
        attrs.by_property_type
          .map(
            (item: any) =>
              `<div class="row"><div><strong>${item.deal_type === "rent" ? "租" : "售"} · ${escapeHtml(item.property_type_label || item.property_type)}</strong><div class="meta">${item.count} 套</div></div></div>`
          )
          .join("")
      )}
      ${listBlock(
  ...
  if (dictionaryButton) {
    dictionaryButton.addEventListener("click", () => {
      openDialog(
        "新增数据字典项",
        `
        <label>字典类型<input name="dict_type" placeholder="customer_source / customer_level / follow_method / payment_method / deal_mode / expense_category / house_source / leave_type / pay_type / property_type / view_feedback" required /></label>
        <label>值<input name="value" required /></label>
