  ...
  if (dictionaryButton) {
    dictionaryButton.addEventListener("click", () => {
      openDialog(
        "新增数据字典项",
        `
        <label>字典类型<input name="dict_type" placeholder="customer_source / customer_level / follow_method / payment_method / deal_mode / expense_category / house_source / leave_type / pay_type / property_type" required /></label>
        <label>值<input name="value" required /></label>
